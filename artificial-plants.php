 ...
 *
 * @wordpress-plugin
 * Plugin Name:       WooCommerce Artificial Plants
 * Plugin URI:        http://whenalive.com
 * Description:       Registering Artificial Plants
 * Version:           1.0.0
 ...
 */
/**
 * Register the artificial plants after init
 */
function register_artificial_plants() {
	/**
	 * This should be in its own separate file.
	 */
	class WC_Product_Artificial_Plants extends WC_Product {
...
add_action( 'plugins_loaded', 'register_artificial_plants' );
...
function artificial_product_tabs( $tabs) {
	$tabs['artificial'] = array(
		...
		'class'		=> array( 'show_if_artificial_plant', 'show_if_variable'  ),
	);
	return $tabs;
}
...
function artificial_options_product_tab_content() {
	...
	?><div id='artificial_options' class='panel woocommerce_options_panel'><?php
		...
		<?php
			...
			woocommerce_wp_text_input(
					array(
							'id'          => '_flowers_artificial_1',
							'label'       => __( 'Flowers', 'woocommerce' ),
							'placeholder' => 'Ex: White Calla Lily, Yellow Tiger Lily',
							'desc_tip'    => 'true',
							'description' => __( 'Enter the flowers.', 'woocommerce' )
					)
			);
...
function save_artificial_option_field( $post_id ) {
	...
	if ( isset( $_POST['_flowers_artificial_1'] ) ) : update_post_meta( $post_id, '_flowers_artificial_1', sanitize_text_field( $_POST['_flowers_artificial_1'] ) ); endif;
	if ( isset( $_POST['_flowers_artificial_2'] ) ) : update_post_meta( $post_id, '_flowers_artificial_2', sanitize_text_field( $_POST['_flowers_artificial_2'] ) ); endif;
	if ( isset( $_POST['_flowers_artificial_3'] ) ) : update_post_meta( $post_id, '_flowers_artificial_3', sanitize_text_field( $_POST['_flowers_artificial_3'] ) ); endif;
	if ( isset( $_POST['_flowers_artificial_4'] ) ) : update_post_meta( $post_id, '_flowers_artificial_4', sanitize_text_field( $_POST['_flowers_artificial_4'] ) ); endif;
...
add_action( 'woocommerce_process_product_meta', 'save_artificial_option_field'  );
